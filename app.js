...
app.post("/addEntrega", (req, res) => {
    ...
    db.get(checkTableQuery, (err, row) => {
      if (err) {
        console.log(err);
        res.status(500).json({ message: "Erro ao verificar tabela no banco de dados" });
        return;
      }
      ...
      if (!row) {
        ...
        db.run(createTableQuery, (err) => {
          if (err) {
            console.log(err);
            res.status(500).json({ message: "Erro ao criar tabela no banco de dados" });
          } else {
  ...
  app.post("/addAtivo", (req, res) => {
    ...
    db.get(checkAtivoQuery, [id], (err, row) => {
      if (err) {
        console.log(err);
        res.status(500).json({ message: "Erro ao verificar ativo no banco de dados" });
        return;
      }
      ...
      if (!row) {
        ...
        db.run(
          ...
          function (err) {
            if (err) {
              console.log(err);
              res.status(500).json({ message: "Erro ao adicionar ativo no banco de dados" });
            } else {
              res.status(200).json({ message: "Ativo adicionado com sucesso", id: id });
            }
          }
        );
      } else {
  ...
  app.get("/listAll", (req, res) => {
    ...
    db.all(getTablesQuery, [], (err, tables) => {
      if (err) {
        console.log(err);
        res.status(500).json({ message: "Erro ao obter tabelas no banco de dados" });
        return;
      }
      ...
      tables.forEach((table) => {
        ...
        db.get(getMaxNumRowQuery, [], (err, row) => {
          if (err) {
            console.log(err);
            res.status(500).json({ message: `Erro ao obter linha de ${tableName}` });
            return;
          }
